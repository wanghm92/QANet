...
def self_attention_block(inputs, num_filters, seq_len, mask = None,
    ...
    with tf.variable_scope(scope, reuse = reuse):
        ...
        outputs = conv(outputs, num_filters, Params.bias, tf.nn.relu, name = "FFN_1", reuse = reuse)
        outputs = conv(outputs, num_filters, Params.bias, None, name = "FFN_2", reuse = reuse)
        outputs = tf.nn.dropout(outputs, 1 - dropout)
        outputs = layer_dropout(outputs, residual, dropout * float(l) / L)
        l += 1
        return outputs, l
