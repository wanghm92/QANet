...
class Model(object):
    ...
    def encode_ids(self):
        with tf.variable_scope("Input_Embedding_Layer"):
            ...
            self.passage_char_encoded = conv(self.passage_char_encoded, Params.num_units,
                bias = True, activation = tf.nn.relu, kernel_size = 5, name = "char_conv", reuse = None)
            self.question_char_encoded = conv(self.question_char_encoded, Params.num_units,
            ...
            self.passage_encoding = tf.nn.dropout(tf.concat((self.passage_word_encoded, self.passage_char_encoded), axis = -1), 1.0 - self.dropout)
            self.question_encoding = tf.nn.dropout(tf.concat((self.question_word_encoded, self.question_char_encoded), axis = -1), 1.0 - self.dropout)

            self.passage_encoding = highway(self.passage_encoding, size = Params.num_units, scope = "highway", reuse = None)
            self.question_encoding = highway(self.question_encoding, size = Params.num_units, scope = "highway", reuse = True)
    ...
    def summary(self):
        ...
        # self.summary_loss = tf.Variable(tf.constant(10.0, shape=(), dtype = tf.float32),trainable=False, name="loss")
        # self.summary_loss_placeholder = tf.placeholder(tf.float32, shape = (), name = "loss_placeholder")
        self.dev_loss = tf.Variable(tf.constant(10.0, shape=(), dtype = tf.float32),trainable=False, name="dev_loss")
        self.dev_loss_placeholder = tf.placeholder(tf.float32, shape = (), name = "dev_loss_placeholder")
        # self.train_loss = tf.Variable(tf.constant(10.0, shape=(), dtype = tf.float32),trainable=False, name="train_loss")
        # self.train_loss_placeholder = tf.placeholder(tf.float32, shape = (), name = "train_loss_placeholder")
        self.metric_assign = tf.group(tf.assign(self.F1, self.F1_placeholder),
            tf.assign(self.EM, self.EM_placeholder),
            tf.assign(self.dev_loss, self.dev_loss_placeholder))
            # tf.assign(self.summary_loss, self.summary_loss_placeholder))
            # tf.assign(self.train_loss, self.train_loss_placeholder))
        tf.summary.scalar('loss_training', self.mean_loss)
        tf.summary.scalar('loss_dev', self.dev_loss)
        # tf.summary.scalar('cross_entropy_loss', self.summary_loss)
